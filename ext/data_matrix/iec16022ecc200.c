...
// return 1 if OK, less than 0 if failed. Does all necessary padding to tl
char ecc200encode(unsigned char *t, int tl, unsigned char *s, int sl, char *encoding, int *lenp) {
  ...
  if (strlen(encoding) < sl) {
    free(encoding);
    return IEC16022ECC200_ERROR_STRING_TOO_SHORT;
  }
  ...
  while (sp < sl && tp < tl) {
    ...
    switch (newenc) { // encode character
      ...
      case 'x':                // X12
         {
            ...
            do {
              ...
              if (c & 0x80) {
                if (newenc == 'x') {
                  free(encoding);
                  return IEC16022ECC200_ERROR_CANNOT_ENCODE_CHARACTER_X12;
                }
                c &= 0x7f;
                out[p++] = 1;
                out[p++] = 30;
                ...
                if (c < 32) { // shift 1
                  ...
                  if (w) { // shift 2
                    ...
                    if (w) {
                      ...
                      free(encoding);
                      return IEC16022ECC200_ERROR_UNEXPECTED_FAILURE;
                    }
                  }
                }
              }
      ...
      case 'e':                // EDIFACT
        {
          ...
          while (sp < sl && tolower(encoding[sp]) == 'e' && p < 4) out[p++] = s[sp++];
          if (p < 4) {
            out[p++] = 0x1F;
            enc = 'a';
          } // termination
      ...
      case 'b':                // Binary
        {
          ...
          if (encoding)
            for (int p = sp; p < sl && tolower(encoding[p]) == 'b'; p++) l++;
          t[tp++] = 231;      // base256
          if (l < 250)
            t[tp++] = l;
          else {
      ...
      default:
        free(encoding);
        return IEC16022ECC200_ERROR_UNKNOWN_ENCODING; // failed
    }
  }
  if (lenp)
    *lenp = tp;
  ...
  if (tp > tl || sp < sl) return IEC16022ECC200_ERROR_DID_NOT_FIT;                 // did not fit
  return 1;                    // OK
}
...
unsigned char* iec16022ecc200(int *Wptr, int *Hptr, char **encodingptr, int barcodelen, unsigned char *barcode, int *lenp, int *maxp, int *eccp) {
  ...
  // we over compensate and check that the input is within this length to avoid any buffer overflow conditions.
  ...
  struct ecc200matrix_s *matrix;
  // GS
  // using the length from the 144x144 semacode in the matrix I don't trust the 3116 maximum length value ... and
  // besides how many characters do you really need in a semacode?
